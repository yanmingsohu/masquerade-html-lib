...
function do_script(script, next, buf, context, errorHandle) {
  ...
  function _require(module_name) {
    ...
    var ref_script = context.filepool.getScript(
        null, null, file, from_context_argv, from_callback_argv);
  ...
  if (atserver) {
    ...
    if (taginfo.selfend) {
      ...
      return function(next, buf, context) {
        ...
        var script = context.filepool.getScript(
            null, null, src, from_context_argv, from_callback_argv);
      ...
      return function(next, buf, context, tag_over) {
        ...
        var script = context.filepool.getScript(
            code, id, filename, from_context_argv, from_callback_argv);
        do_script(script, next, buf, context, errorHandle);
      };
    }
